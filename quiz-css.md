...
答：
1.Can I use CSS3浏览器兼容支持情况查询工具
2.OOCSS是指面向对象的CSS，其核心概念是采用模块化的实现方式，将CSS分解成可复用和继承的对象，OOCSS 的目的是鼓励代码复用。
1）结构与皮肤分离
2）内容与容器分离
3.bootstrap栅格系统布局。bootstrap把页面设置为12列。通过改变列的数字来进行布局来适应不同媒体桌面。